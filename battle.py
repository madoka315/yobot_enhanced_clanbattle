...
class ClanBattle:
    ...
    def notify_subscribe(self, group_id: Groupid, boss_num=None, send_private_msg=False):
        ...
        # 如果开启了提前提醒模式，则加上下一个boss的预约提醒
        notify_in_advance = rLoadSettings(group_id, 'notify_in_advance')
        pre_notice = []
        if notify_in_advance:
            boss_num += 1
            if boss_num == 6:
                boss_num = 1
            for subscribe in Clan_subscribe.select().where(
                ...
                if subscribe.message:
                    msg += subscribe.message
                pre_notice.append(msg)
        #
        ######
        message = ''
        if notice or pre_notice:
            message = 'boss寄了\n'
            if notice:
                message += '\n'.join(notice)
            if pre_notice:
    ...
    def execute(self, match_num, ctx):
        ...
        if match_num == 1:  # 创建
            ...
            try:
                ...
                return str(e)
            #######    
            multi_type = rLoadSettings(group_id, "multi_challenging_mode") 
            multi_challenging_info = ''
            if multi_type: 
                info = rLoadChallenge(group_id)
                niks = []
                extra_msgs = list(info[1])
                group = Clan_group.get_or_none(group_id=group_id)
                pcrdate = pcr_datetime(group.game_server)[0]
                for id in info[0]:
                    # 查询是否是补偿，直接显示在昵称前面
                    tip = ''
                    knifes = self.get_report(group_id, None, id, pcrdate)
                    for i,knife in enumerate(knifes):
                        if i+1 != len(knifes):
                            continue
                        if knife['health_ramain'] == 0 and knife['is_continue'] == False:
                            cycle = knife['cycle']
                            boss_num = knife['boss_num']
                            tip = f'({cycle}-{boss_num}补偿刀)'
                    #
                    niks.append(tip+self._get_nickname_by_qqid(id))
                if niks != []:
                    multi_challenging_info = '\n==以下成员正在挑战=='
                    for i in range(0,len(niks)):
                        multi_challenging_info += f'\n{niks[i]}'
                        if extra_msgs[i] != '':
                            multi_challenging_info += f'：{extra_msgs[i]}'
                    multi_challenging_info += '\n====='        
            try:
                (
                    full_challenge_count,
            ...
            return f'{url}{boss_summary}{multi_challenging_info}{progress}'
        elif match_num == 4:  # 报刀
            match = re.match(
            ...
            try:
                ...
                _logger.info('群聊 失败 {} {} {}'.format(user_id, group_id, cmd))
                return str(e)
            ########## 报刀时如果是多人出刀模式，删除报刀者申请信息    
            multi_type = rLoadSettings(group_id, "multi_challenging_mode")
            if multi_type:
                rCancleChallenge(group_id, user_id)
            if behalf:
            ...
            try:
                ...
                _logger.info('群聊 失败 {} {} {}'.format(user_id, group_id, cmd))
                return str(e)
            ########## 尾刀时如果是多人出刀模式，删除群组申请信息    
            multi_type = rLoadSettings(group_id, "multi_challenging_mode")
            if multi_type:
                rCancleChallenge(group_id)    
            if behalf:
            ...
            tip = ''
            pcrdate = pcr_datetime(group.game_server)[0]
            knifes = self.get_report(group_id, None, None, pcrdate)
            # 先统计，如果finished是.5，则表明是没出的补偿
            # sorted = {
            #   qqid:{
            #       'finished':int,
            #       'detail':[challenges]
            ...
            for i in knifes:
                ...
                # 第0.5刀放入index1,第1刀放入index2...   
                index = int(2 * sorted[qqid]['finished'])
                sorted[qqid]['detail'][index] = i  
                if i['is_continue']:
                    sorted[qqid]['finished'] += 0.5
                elif i['health_ramain'] != 0:
                    sorted[qqid]['finished'] += 1
                else:
                    sorted[qqid]['finished'] += 0.5
      
            next_boss = boss_num+1
            if next_boss == 6:
                next_boss = 1   
            for key,values in sorted.items():
                if values['finished'] % 1 != 0:
                    index = int(values['finished'] *2-1)
                    knife = values['detail'][index]
                    if knife['boss_num'] == next_boss:
                        cycle = knife['cycle']
                        boss_num = knife['boss_num']
                        damage = knife['damage']
                        nickname = self._get_nickname_by_qqid(key)
                        tip += f'\n{nickname}：{cycle}-{boss_num} 收尾{damage}hp的补偿刀'
            if tip:
                tip = f'\n=={next_boss}王存在补偿==' + tip
            if counts == 0:
                return str(boss_status) + tip
            else:
                return str(boss_status) + '\n※已自动取消该boss的预约' + tip
        elif match_num == 6:  # 撤销
            if cmd != '撤销':
            ...
            try:
                ...
                return str(e)
            _logger.info('群聊 成功 {} {} {}'.format(user_id, group_id, cmd))
            ######## 查询预约目标的人数，如果加上这次预约达到4人则发出提醒
            subscribers = self.get_subscribe_list(group_id, boss_num)
            reply = ''
            if len(subscribers) >= 4:
                reply = f'\n*当前boss预约已达{len(subscribers)}人：'
        ...
        ##########
            ...
            if match:
                ...
                if isinstance(extra_msg, str):
                    ...
                    return
            # 没有开启多人出刀，或者是锁定行为，使用原有处理
            multi_type = rLoadSettings(group_id, "multi_challenging_mode")
            if multi_type == False or multi_type == None or appli_type == 2:
                try:
                ...
                if isinstance(extra_msg, str):
                    ...
                    if not extra_msg:
                        extra_msg = None
                try:
                    info = rApplyForChallenge(group_id, user_id, extra_msg)
                    msg = info[0]
                    nik = self._get_nickname_by_qqid(info[1]) if info[1] else None
                    if '失败' in msg:
                        msg += f'\n锁定者：{nik}'
                    elif '重复' in msg:
                        pass
                    ...
                    _logger.info('群聊 失败 {} {} {}'.format(user_id, group_id, cmd))
                    return str(e)
        elif match_num == 30:  # 更新出刀
            match = re.match(r'^更新出刀 *(?:[\:：](.*))?$', cmd)
            if not match:
                return
            multi_type = rLoadSettings(group_id, "multi_challenging_mode")
            if not multi_type:
                return '本群未开启多人出刀模式，不可使用'
            extra_msg = match.group(1)
            if isinstance(extra_msg, str):
                extra_msg = extra_msg.strip()
                if not extra_msg:
                    return '请输入需要更新的信息'
            msg = rUpdateForChallenge(group_id, user_id, extra_msg)
            multi_challenging_info = ''
            if '成功' in msg:
                info = rLoadChallenge(group_id)
                niks = []
                ...
                if niks != []:
                    ...
                    for i in range(0,len(niks)):
                        ...
                        if extra_msgs[i] != '':
                            multi_challenging_info += f'：{extra_msgs[i]}'
            return msg+multi_challenging_info    
        ...
        ###########
            ...
            if behalf:
                user_id = behalf
                _logger.info(f'{user_id}')
            try:
                group = Clan_group.get_or_none(group_id=group_id)
                pcrdate = pcr_datetime(group.game_server)[0]
                _logger.info(f'群聊 查询日期{pcrdate}')
                knifes = self.get_report(group_id, None, user_id, pcrdate)
                user = User.get_or_create(
                    qqid=user_id,
                    defaults={
                    'clan_group_id': group_id,
                ...
                for i in knifes:
                    flag = ''
                    if i['health_ramain'] == 0 and i['is_continue'] == False:
                        num += 0.5
                        flag = '(尾刀)'
                    elif i['is_continue'] == True:
                        num += 0.5
                        flag = '(补偿刀)'
                    else:
                        num += 1
                    ...
                    boss_num = i['boss_num']
                    damage = i['damage']
                    reply += f'{cycle}周目{boss_num}王 造成{damage}伤害{flag}\n'
                if num < 3:
                    reply += f'您还差{3-num}刀下班，加油！\n'
                elif num == 3:
                    reply += '今日出刀已完成！\n'
                if num == 0:
                    reply += '尚未出刀\n'
            ...
            match = re.match('^开启(多人出刀|预约提前提醒|提前预约提醒|提前提醒预约)$', cmd)
            if not match:
                return
            user = User.get_or_create(
                qqid=user_id,
            ...
            if (ctx['sender']['role'] == 'member') and (user.authority_group >= 100):
                return '只有管理员才可以开关特殊公会战模式'
            if cmd == '开启多人出刀':
                mode = 'multi_challenging_mode'
                # 开启多人出刀时强制解锁单人模式的申请/锁定
                try:
                    self.cancel_application(group_id, user_id)
                except GroupNotExist as e:
                    _logger.info('群聊 失败 {} {} {}'.format(user_id, group_id, cmd))
                    return str(e)
                except GroupError:
                ...
                mode = 'notify_in_advance'    
            try:
                rSaveSettings(group_id, mode, True)
                _logger.info('群聊 成功 {} {} {}'.format(user_id, group_id, cmd))
                return '本群公会战出刀模式已设为多人出刀模式' if mode == 'multi_challenging_mode' else '本群已设置预约提前提醒模式'
            except GroupNotExist as e:
                return str(e)
        elif match_num == 92:
            match = re.match('^关闭(多人出刀|预约提前提醒|提前预约提醒|提前提醒预约)$', cmd)
            if not match:
                return
            user = User.get_or_create(
                qqid=user_id,
            ...
            if cmd == '关闭多人出刀':
                ...
                mode = 'notify_in_advance'
            try:
                rSaveSettings(group_id, mode, False)
                _logger.info('群聊 成功 {} {} {}'.format(user_id, group_id, cmd))
                return '本群公会战出刀模式已设为单人出刀模式' if mode == 'multi_challenging_mode' else '本群已关闭预约提前提醒模式'   
            except GroupNotExist as e:
                return str(e)
